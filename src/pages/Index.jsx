...
import { Wrench, Droplet, Car, Home, Search, Zap, Trash, Wallet, Star, Gift, Heart, Clock, ThumbsUp } from 'lucide-react';
import { Link } from 'react-router-dom';

const services = [
  { id: 1, name: 'Air Conditioning Repair', icon: Wrench, description: 'Fix or maintain your AC unit', category: 'Home Maintenance', rating: 4.5 },
  { id: 2, name: 'Plumbing Services', icon: Droplet, description: 'Resolve any plumbing issues', category: 'Home Maintenance', rating: 4.2 },
  { id: 3, name: 'Car Wash', icon: Car, description: 'Get your car cleaned and shining', category: 'Automotive', rating: 4.7 },
  { id: 4, name: 'House Cleaning', icon: Home, description: 'Professional house cleaning services', category: 'Cleaning', rating: 4.8 },
  { id: 5, name: 'Electrical Services', icon: Zap, description: 'Electrical repairs and installations', category: 'Home Maintenance', rating: 4.3 },
  { id: 6, name: 'Waste Removal', icon: Trash, description: 'Efficient waste removal and disposal', category: 'Home Maintenance', rating: 4.1 },
];
  ...
  return (
    <Card className="w-full h-full flex flex-col hover:shadow-lg transition-shadow duration-300">
      ...
      <CardContent className="pb-2 flex-grow">
        ...
        <p className="text-sm mt-2 font-semibold text-blue-600">{service.category}</p>
        {isLoading ? (
          <p className="text-sm mt-2">Loading available providers...</p>
        ) : (
          <div className="mt-2">
            <p className="text-sm font-semibold">{availableProviders} providers available</p>
      ...
      <CardFooter className="flex justify-between items-center">
        <Link to={`/request/${service.id}`} className="flex-grow">
          <Button className="w-full bg-blue-500 hover:bg-blue-600">Book Service</Button>
        </Link>
...
const Sidebar = () => (
  <div className="bg-gray-100 p-4 rounded-lg space-y-4">
    ...
    <Button variant="ghost" className="w-full justify-start hover:bg-yellow-100">
    ...
    <ul className="space-y-2">
      {services.slice(0, 3).map((service) => (
        <li key={service.id} className="flex items-center">
          <service.icon className="h-4 w-4 text-blue-500 mr-2" />
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <h1 className="text-4xl font-bold text-center mb-8 text-blue-600">Welcome to Tackld</h1>
