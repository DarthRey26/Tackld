-- Add missing indexes for performance (without CONCURRENTLY due to transaction limitations)
CREATE INDEX IF NOT EXISTS idx_bids_expires_at ON public.bids(expires_at);
CREATE INDEX IF NOT EXISTS idx_bids_contractor_id_status ON public.bids(contractor_id, status);
CREATE INDEX IF NOT EXISTS idx_bids_booking_id_status ON public.bids(booking_id, status);
CREATE INDEX IF NOT EXISTS idx_bookings_current_stage ON public.bookings(current_stage);
CREATE INDEX IF NOT EXISTS idx_bookings_service_contractor ON public.bookings(service_type, contractor_id);
