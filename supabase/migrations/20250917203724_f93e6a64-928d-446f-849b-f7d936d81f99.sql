-- AUDIT AND FIX DATABASE ISSUES (WITHOUT CONCURRENT INDEXES)
...
-- Create performance indexes (non-concurrent)
CREATE INDEX IF NOT EXISTS idx_bids_booking_status ON public.bids(booking_id, status);
CREATE INDEX IF NOT EXISTS idx_bids_contractor_status ON public.bids(contractor_id, status);
CREATE INDEX IF NOT EXISTS idx_bids_expires_at ON public.bids(expires_at) WHERE status = 'pending';
CREATE INDEX IF NOT EXISTS idx_bookings_service_status ON public.bookings(service_type, status) WHERE contractor_id IS NULL;
